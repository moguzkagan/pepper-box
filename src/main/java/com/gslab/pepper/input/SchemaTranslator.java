...
/**
 * The SchemaTranslator class gets series of java statements and generates
 * iterator using class template
 *
...
public class SchemaTranslator {
	...
	public Iterator getPlainTextMsgIterator(String schemExecStatement) throws PepperBoxException {
		...
		try {
			...
			final String messageGeneratorSource = getTemplateFileContent(PropsKeys.PLAINTEXT_MESSAGE_GENERATOR_TPL)
					.replace(PropsKeys.MSG_GEN_PLC_HLDR, schemExecStatement).replace(PropsKeys.JAVA_CLS_PLC_HLDR, classname);
			...
			log.error("Failed to compileSchemaClass class", e);
	...
	public Iterator getSerializedMsgIterator(String pojoClass, String execStatement) throws PepperBoxException {
		...
		try {
			...
			// Replace placeholders in template and get java source code
			final String messageGeneratorSource = getTemplateFileContent(PropsKeys.SERIALIZED_MESSAGE_GENERATOR_TPL)
					.replace(PropsKeys.MSG_GEN_PLC_HLDR, execStatement).replace(PropsKeys.JAVA_CLS_PLC_HLDR, classname)
					.replace(PropsKeys.OBJ_CLASS, pojoClass);
