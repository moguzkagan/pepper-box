...
/**
 * The FieldDataFunctions is is set of builtin template function and can be
 * invoked within {{ }}
 *
...
public class FieldDataFunctions {
	...
	private static final Logger log = LogManager.getLogger(FieldDataFunctions.class.getName());
	 ...
	 * 
	 * @param beginDate start date for random timestamp in dd-MM-yyyy HH:mm:ss
	 *                  format
	...
	public static long TIMESTAMP(String beginDate, String endDate) {
		...
		try {
			...
			log.error("Failed to parse range date", e);
	...
	public static String IPV4() {
		...
		try {
			...
			log.error("Failed to generate IPV4", e);
		}
		return randomIPV4;
	}
	...
	static {
		...
		try {
			...
			log.error("failed to load first name, last name CSV files", exc);
